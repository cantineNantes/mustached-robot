...
class Less_Core
{
	...
	public static function compile($array = array(), $media = 'screen')
	{
		...
		{ 
			...
			{
				array_push($assets, self::_html_comment('could not find '.$file.'.less'));
			}
		}
		...
		// get less config
		$config = Config::load('less');

		// if compression is allowed
		if ($config['compress'])
		{
			return Fuel\Core\Asset::css(self::_combine($stylesheets, $config), array('media' => $media));
		}
		...
		{ 
			$filename = self::_get_filename($file, $config['path']);
                        array_push($assets, Fuel\Core\Asset::css($filename, array('media' => $media)));
		}
	...
	protected static function _get_filename($file, $path)
	{
		...
		$filename = $filename.'-'.$last_modified.'.css';
		...
		if ( ! file_exists($path.$filename))
		{
			...
			touch($path.$filename, filemtime($file.'.less') - 3600);

			lessc::ccompile($file.'.less', $path.$filename);
		}
	 ...
	 * @return  string   path to the asset file
	 */
	protected static function _combine($files, $config)
	{
		...
		$filename = md5(implode('|', $files)).'-'.$last_modified.'.css';

		// if the file exists no need to generate
		if ( ! file_exists($filename))
		{
			self::_generate_assets($config['path'].$filename, $files);
		}
